...
void cairo_read_image(char * filename, struct cairo_image *image)
{
    ...
    image->format = cairo_image_surface_get_format(image->surface);

    if ( image->surface != CAIRO_STATUS_SUCCESS ) {

        image->data = stbi_load(filename, &image->width, &image->height, &image->n_components, 4);
        if ( image->data == NULL )
        ...
        image->surface = cairo_image_surface_create_for_data(image->data, image->format, image->width, image->height, image->stride);
    }
}
